...
// fetch to check if the entered State is correct
// is doing the typo check on the backend
...
//checks if states arrays has 50 States to keep adding or run the calculate()
function addLiToUl(state) {
  if (userStates.length < 50) {
    // grab ul element
    var ulStateElement = document.getElementById('state-ul');
    // grab ul column-one to list the states entered by the user
    var ulElement = document.getElementById('column-one');
    ...
    liElement.innerText = userStates.length + 1 + '- ' + state;
  ...
  if (userStates.length === 50) {
    // if the array has 50 elements add the submit button
    // addSubmitButton()
    // now instead of adding we directly execute the calculate function
    // if the array has 50 elements then run
    calculate();
  }
}

// add a button submit to send the States array entered by the user
...
// Compare is the array States entered matches the list of Oficial States.
function compareArrays(states) {
  ...
  var result = function result(currentState) {
    return userStates.includes(currentState.name)(states.every(result)) ? alert("You are a SMARTYPANTS") : alert('You lose!');
  };
}
...
function countDown() {
  // variables to reset the timer when time runs out (converted to integer)
  ...
  // variables to decrease the time
...
//Reset the array userStates
function clearStatesArray() {
  userStates = [];
}
